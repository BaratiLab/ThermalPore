...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, config, indexes = None, dev = False):
        ...
        self.pores = None
        self.pore_ids = None
        self.equivalent_diameter_voxel = None
        self.equivalent_diameter_pore = None
        self.load_ct()
    ...
    def __getitem__(self, index):
        ...
        frame_files = self.layer_frames[index]
        ...
        for frame_file in frame_files[:self.config["frame_length"]]:
            ...
            frame_data = open(
                f"{self.datasets_dir}/beta=0.7_pickles/{frame_file}", "rb"
            )
        ...
        ct_build_layers = self.get_build_layer(index)
    ...
    def load_ct(self):
        ...
        # Voxel Data
        self.sample = h5_file["CTDataContainer"]["VoxelData"]["Sample"]
        self.segmented = h5_file["CTDataContainer"]["VoxelData"]["Segmented"]
        self.pores = h5_file["CTDataContainer"]["VoxelData"]["Pores"]
        self.pore_ids = h5_file["CTDataContainer"]["VoxelData"]["PoreIds"]
        self.equivalent_diameter_voxel = h5_file["CTDataContainer"]["VoxelData"]["EqDiam_Vox"]

        # Pore Data
        self.equivalent_diameter_pore = h5_file["CTDataContainer"]["PoreData"]["EquivalentDiameters"]
    ...
    def load_pyrometry(self):
        ...
        with open(f"{self.datasets_dir}/pyrometry_map.p", "rb") as map_file:
            self.pyrometry_map = pickle.load(map_file)

        # List of frame files associated with each one of the 159 build layers.
        self.layer_frames = self.pyrometry_map["layer_frames"]
    ...
    def get_build_layer(self, layer_index = 0):
        """
        ...
        ct_z_stop_index = ct_voxel_index + abs(BUILD_LAYER_OFFSETS[self.config["dataset"]][0])
        ...
        layer_ct_sample = self.sample[
            ct_z_start_index:ct_z_stop_index,
            ct_y_start_index:ct_y_stop_index,
            ct_x_start_index:ct_x_stop_index,
        ]

        layer_ct_segmented = self.segmented[
            ct_z_start_index:ct_z_stop_index,
            ct_y_start_index:ct_y_stop_index,
            ct_x_start_index:ct_x_stop_index,
        ]

        layer_ct_pores = self.pores[
            ct_z_start_index:ct_z_stop_index,
            ct_y_start_index:ct_y_stop_index,
            ct_x_start_index:ct_x_stop_index,
        ]

        layer_ct_pore_ids = self.pore_ids[
            ct_z_start_index:ct_z_stop_index,
            ct_y_start_index:ct_y_stop_index,
            ct_x_start_index:ct_x_stop_index,
        ]

        layer_ct_equivalent_diameter_voxel = self.equivalent_diameter_voxel[
            ct_z_start_index:ct_z_stop_index,
            ct_y_start_index:ct_y_stop_index,
            ct_x_start_index:ct_x_stop_index,
        ]
...
def split_dataset(train_fraction, test_fraction, verbose = False):
